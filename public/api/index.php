...
ini_set('display_errors', 0); // Keep 0 for production JSON output
...
// 4. AUTO-MIGRATION (Check if tables exist, if not create them)
function checkAndCreateTables($mysqli) {
    ...
    if ($check->num_rows == 0) {
        ...
        ";
        // MySQLi multi_query is needed for multiple statements
        $mysqli->multi_query($sql);
        while ($mysqli->next_result()) {;} // flush results
    }
}

// Run migration check silently
...
try {
    ...
    if ($action === 'register') {
        ...
        if ($stmt->execute()) {
            ...
            sendJson('error', 'Insert failed: ' . $mysqli->error);
        }
    }
    ...
    // --- PING ---
    elseif ($action === 'ping') {
        sendJson('success', 'Connected', ['host' => $mysqli->host_info]);
    }
    ...
    sendJson('error', 'Server Error', null, $e->getMessage());
}
